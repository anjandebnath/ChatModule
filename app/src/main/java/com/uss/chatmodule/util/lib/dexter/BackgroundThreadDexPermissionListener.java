 ...
 */
public class BackgroundThreadDexPermissionListener extends DexPermissionListener {

  private Handler handler = new Handler(Looper.getMainLooper());

  public BackgroundThreadDexPermissionListener(BaseActivity activity) {
    super(activity);
  }
  ...
  public void onPermissionGranted(final PermissionGrantedResponse response) {
    handler.post(new Runnable() {
      @Override
      public void run() {
        BackgroundThreadDexPermissionListener.super.onPermissionGranted(response);
      }
    });
  }
  ...
  public void onPermissionDenied(final PermissionDeniedResponse response) {
    handler.post(new Runnable() {
      @Override
      public void run() {
        BackgroundThreadDexPermissionListener.super.onPermissionDenied(response);
      }
    });
  }
  ...
  public void onPermissionRationaleShouldBeShown(final PermissionRequest permission,
    ...
    handler.post(new Runnable() {
      @Override
      public void run() {
        BackgroundThreadDexPermissionListener.super.onPermissionRationaleShouldBeShown(
            permission, token);
      }
    });
  }
